...
class Post {
	...
	public function submitPost($body, $user_to) {
		...
		if($check_empty != "") {
			...
			//Insert post
			$query = mysqli_query($this->con, "INSERT INTO posts VALUES('0', '$body', '$added_by', '$user_to', '$date_added', 'no', 'no', '0')");
	...
	public function loadPostsFriends($data, $limit) {
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				//Prepare user to string so it can be included even if not posts 
				if($row['user_to'] == "none") {
					$user_to = "";
				}
				else {
